...
public class Compiler {
	...
	private void verifyMethodsParams(ArrayList<Variable> paramList, ArrayList<Expr> exprList) {
		...
		for (int i = 0; i < sizeParam; i++) {
			...
			if (!var.getType().getName().equals(expr.getType().getName())) {
				...
				TypeCianetoClass superClass = semanticChecking.getSuperClass();
				if(superClass!=null) {
					if(superClass.isSubtype(var.getType().getName())) {
						isSubtype = true;
					}
				}
				
				if(!isSubtype) {
					TypeCianetoClass c = semanticChecking.getTypeCianetoClass(expr.getType().getName());
					if(c!=null) {
						if(c.isSubtype(var.getType().getName())) {
							isSubtype = true;
						}
					}
				}
	...
	private Type verifyArgumentsOfSendMessage(String methodName, MethodDec method, ArrayList<Expr> exprList) {
		...
		for (int i = 0; i < sizeParam; i++) {
			...
			Expr expr = exprList.get(i);
			if (!var.getType().getName().equals(expr.getType().getName())) {
				if(!isSubtype(var.getType(), expr.getType()) || !isSubtype(expr.getType(), var.getType())) {
	...
	private boolean isSubtype(Type t1, Type t2) {
		...
		String currentClassName = semanticChecking.getCurrentClassName();
		
		if(currentClassName.equals(t1.getName())) {
			// t1 eh a classe atual
			TypeCianetoClass superClass = semanticChecking.getSuperClass();
			if(superClass!=null && superClass.isSubtype(t2.getName())) {
				return true;
			}
		} else {
			if(currentClassName.equals(t2.getName())) {
				TypeCianetoClass superClass = semanticChecking.getSuperClass();
				if(superClass!=null && superClass.isSubtype(t1.getName())) {
					return true;
				}
			} else {
				TypeCianetoClass classT1 = semanticChecking.getTypeCianetoClass(t1.getName());
				TypeCianetoClass classT2 = semanticChecking.getTypeCianetoClass(t2.getName());
				if(classT1.isSubtype(classT2.getName()) || classT2.isSubtype(classT1.getName())) {
					return true;
				}
			}
		}
